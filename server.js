...
const SERVICE_NAME = 'newtaipei-noise-control-system-v66-single-richmenu-force-refresh-autosync';
...
function lineRichMenuPayload(page = 'main') {
  // V66 單頁版 Rich Menu：直接使用使用者指定圖片。
  // 圖檔已壓縮至 LINE 限制內；不分兩頁、不使用上下掀頁。
  const w = 2500, h = 1686;
  return {
    size: { width: w, height: h },
    selected: true,
    name: 'NTPC Noise V66 單頁 Rich Menu',
    chatBarText: '新北噪音車',
    areas: [
      // 上方六大功能
      area(35, 405, 410, 530, { type: 'uri', uri: FIELD_REPORT_URL }),
      area(455, 405, 410, 530, { type: 'uri', uri: DASHBOARD_URL }),
      area(875, 405, 410, 530, { type: 'message', text: '即時機況' }),
      area(1295, 405, 410, 530, { type: 'message', text: '設備管理' }),
      area(1715, 405, 410, 530, { type: 'uri', uri: CASE_TRACKING_URL }),
      area(2130, 405, 335, 530, { type: 'message', text: '統計查詢' }),
      // 下方法規資訊四大功能
      area(80, 1105, 540, 355, { type: 'message', text: '法規中心' }),
      area(655, 1105, 540, 355, { type: 'uri', uri: HOTSPOT_URL }),
      area(1230, 1105, 540, 355, { type: 'message', text: '陳情趨勢' }),
      area(1805, 1105, 540, 355, { type: 'message', text: '新聞' })
    ]
  };
}
...
async function updateRichMenu() {
  ...
  return {
    ok: !!setDefault.ok,
    mode: 'single-page-rich-menu-v66-force-refresh',
    mainRichMenuId,
