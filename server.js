...
const SERVICE_NAME = 'newtaipei-noise-control-system-v67-hightech-dashboard-single-richmenu-autosync';
...
function lineRichMenuPayload(page = 'main') {
  // V67 單頁版 Rich Menu：科技儀表板式介面。
  // 圖檔只使用 public/assets/line-rich-menu-main.jpg。
  const w = 2500, h = 1686;
  return {
    size: { width: w, height: h },
    selected: true,
    name: 'NTPC Noise V67 高科技單頁 Rich Menu',
    chatBarText: '新北噪音車',
    areas: [
      // 右側主要六大功能
      area(900, 65, 485, 440, { type: 'uri', uri: FIELD_REPORT_URL }),
      area(1430, 65, 485, 440, { type: 'uri', uri: DASHBOARD_URL }),
      area(1940, 65, 500, 440, { type: 'message', text: '即時機況' }),
      area(900, 535, 485, 425, { type: 'message', text: '設備管理' }),
      area(1430, 535, 485, 425, { type: 'uri', uri: CASE_TRACKING_URL }),
      area(1940, 535, 500, 425, { type: 'message', text: '統計查詢' }),

      // 下方法規 / 資訊四大功能
      area(900, 975, 365, 320, { type: 'message', text: '法規中心' }),
      area(1290, 975, 355, 320, { type: 'uri', uri: HOTSPOT_URL }),
      area(1670, 975, 355, 320, { type: 'message', text: '陳情趨勢' }),
      area(2040, 975, 400, 320, { type: 'message', text: '新聞' }),

      // 左側快捷提示區
      area(65, 285, 790, 170, { type: 'uri', uri: FIELD_REPORT_URL }),
      area(65, 455, 790, 165, { type: 'message', text: '即時機況' }),
      area(65, 620, 790, 170, { type: 'uri', uri: CASE_TRACKING_URL })
    ]
  };
}
...
async function updateRichMenu() {
  ...
  return {
    ok: !!setDefault.ok,
    mode: 'single-page-rich-menu-v67-hightech-dashboard-force-refresh',
    mainRichMenuId,
...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/healthz') return json(res, 200, { ok: true, service: SERVICE_NAME, version: APP_VERSION, noInstall: true, hasAdminPassword: !!env('ADMIN_PASSWORD'), hasSessionSecret: !!env('SESSION_SECRET'), time: taipeiTime() });
