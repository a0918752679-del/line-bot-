...
const SERVICE_NAME = 'newtaipei-noise-control-system-v69-bright-tech-richmenu-hidden-admin-autosync';
...
function lineRichMenuPayload(page = 'main') {
  // V69 單頁版 Rich Menu：明亮科技卡片式操作介面。
  // 右下角保留隱藏功能區：連續點三下後啟動後端平台驗證入口。
  const w = 2500, h = 1686;
  return {
    size: { width: w, height: h },
    selected: true,
    name: 'NTPC Noise V69 明亮科技單頁 Rich Menu',
    chatBarText: '新北噪音車',
    areas: [
      // 主要六大功能
...
async function updateRichMenu() {
  ...
  return {
    ok: !!setDefault.ok,
    mode: 'single-page-rich-menu-v69-hidden-admin-force-refresh',
    mainRichMenuId,
