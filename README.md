...
## V68 更新
- 移除左側熱門快捷功能與系統狀態區塊。
- 改為更簡潔的科技儀表板單頁 Rich Menu。
- 主要功能卡片保持大尺寸，避免手機上字太小或誤觸。
- `public/assets` 僅保留最新 `line-rich-menu-main.jpg`、`ntpc-emblem.png`、`README_RICH_MENU.txt`。
- 保留強制刷新舊 Rich Menu 與 Google Sheet 自動同步。
- 新服務名稱：`newtaipei-noise-control-system-v68-hightech-single-richmenu-clean-autosync`


## V69 更新
- 套用新的明亮科技卡片式 Rich Menu。
- 移除系統小幫手區塊。
- 右下角新增隱藏功能：連點三下啟動後端平台入口。
- 後端平台入口新增密碼驗證，預設密碼：`69677323`。
- 建議正式部署時設定：
  - `ADMIN_PASSWORD=69677323`
  - `ADMIN_SESSION_MINUTES=60`
  - `SESSION_SECRET=請設定一組隨機字串`
- 保留強制刷新舊 Rich Menu 與 Google Sheet 自動同步。
- 新服務名稱：`newtaipei-noise-control-system-v69-bright-tech-richmenu-hidden-admin-autosync`
